...
Remediation: Require either PSK authentication or utilizing AP Isolation.
...
### Weak/Default Passwords on Guest Wireless (Medium) (AP/HotSpot Attacks) - The passphrase for accessing the network is weak and easily guessed, which could compromise the system's security.
Recommendation:
Configure the wireless network with a unique, robust, complex passphrase.

### VLAN Escaping (VLAN Hopping) - Many networks tend to have poor VLAN implementation or misconfigurations which could allow an attacker to exploit the VLAN trunking protocol to gain unauthorized access to other VLANs configured on the switch. Many switches are configured to utilize DTP (Dynamic Trunking Protocol) by default. DTP establishing a trunk link between two switches. An attacker conduct a few methods to attempt VLAN escaping.
Recommendation:
It is recommended to implement 802.1x to regulate access to the network and to disable any unused ports. Keep the native VLAN of all trunk ports different from user VLANs and disable the use of DTP on the switches.
